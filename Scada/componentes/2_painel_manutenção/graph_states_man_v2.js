...
for(var i = point_values.size()-1; i >= 0; i--){
    ...
    for(var g=0; g<state_groups.length; g++){
        ...
        datasets[g].pointRadius.unshift(2);
        datasets[g].pointBackgroundColor.unshift(series_colors[g]);
    }
}
...
var s = "<div id='"+div_id+"_legend' style='width:"+width+"px; background:"+background_color+"; text-align:center; margin-bottom:2px;'>";
for(var g=0; g<series_colors.length; g++){
    s += "<span data-ds='"+g+"' style='cursor:pointer; display:inline-block; width:20px; height:20px; border-radius:50%; margin:0 6px; background:"+series_colors[g]+";'></span>";
}
s += "</div>";
...
s += "     },";
s += "     scales:{";
s += "       x:{ type:'linear', ticks:{ callback:function(val){ var d=new Date(val); return d.getHours().toString().padStart(2,'0')+':'+d.getMinutes().toString().padStart(2,'0'); }, maxTicksLimit:12 }, offset:false },";
...
s += "         var i = el.index;";
s += "         chart.data.datasets.forEach(function(ds){";
s += "           ds.pointRadius = ds.data.map(()=>2);";
s += "           ds.pointBackgroundColor = ds.data.map(()=> ds.borderColor);";
s += "         });";
s += "         chart.data.datasets[dsIndex].pointRadius[i] = 8;";
...
s += "   }";
s += " });";
s += " document.querySelectorAll('#"+div_id+"_legend span').forEach(function(el){";
s += "   el.addEventListener('click',function(){";
s += "     var idx = parseInt(this.getAttribute('data-ds'));";
s += "     chart.data.datasets[idx].hidden = !chart.data.datasets[idx].hidden;";
s += "     chart.update();";
s += "   });";
s += " });";
s += "}";
